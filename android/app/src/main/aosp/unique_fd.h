...
extern void
android_fdsan_exchange_owner_tag(int fd, uint64_t expected_tag, uint64_t new_tag) __INTRODUCED_IN(
        29) __attribute__((__weak__));

extern uint64_t
android_fdsan_create_owner_tag(enum android_fdsan_owner_type type, uint64_t tag) __INTRODUCED_IN(
        29) __attribute__((__weak__));

extern int
android_fdsan_close_with_tag(int fd, uint64_t tag) __INTRODUCED_IN(29) __attribute__((__weak__));
...
namespace android {
    namespace base {
        ...
        private:
            ...
            template<typename T = Closer>
            static auto
            close(int fd, void *tag_value) -> decltype(T::Close(fd, tag_value), void()) {
...
// Inline functions, so that they can be used header-only.
        ...
        template<typename Closer>
        inline bool
        Socketpair(int type, unique_fd_impl<Closer> *left, unique_fd_impl<Closer> *right) {
