...
bool ReadFdToString(android::base::borrowed_fd fd, std::string *content) {
    content->clear();
...
static std::string get_ashmem_device_path() {
    ...
    std::string boot_id;
    if (!ReadFileToString(boot_id_path, &boot_id, false)) {
...
int ashmem_create_region(const char *name, size_t size) {
    ...
    if (has_memfd_support()) {
        ALOGD("ashmem_create_region: %s", "has_memfd_support yes");
        return memfd_create_region(name ? name : "none", size);
    }
    ALOGD("ashmem_create_region: %s", "has_memfd_support no");
